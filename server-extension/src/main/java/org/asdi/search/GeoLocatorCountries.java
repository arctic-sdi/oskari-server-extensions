 ...
 */
public class GeoLocatorCountries {
    ...
    private static GeoLocatorCountries self = null;

    public static GeoLocatorCountries getInstance() {
        if(self == null) {
            self = new GeoLocatorCountries();
        }
        return self;
    }

    private GeoLocatorCountries() {
        String urlProp = "search.channel." + GeoLocatorSearchChannel.ID + ".service.url";
        serviceURL = PropertyUtil.getOptional(urlProp);
        if (serviceURL == null) {
            log.warn("ServiceURL not configured. Add property with key", urlProp);
        }
        loadCountryMap();
    }
